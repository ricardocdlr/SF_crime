...
def append_vector(array_x, array_y, point):
	...
	for a in xrange(MAX_SFPD):
		if (a == point['SFPD_incidents']):
			vector_y.append(1.0)
		else:
			vector_y.append(0.0)
...
def main():
	...
	y_expected = tf.placeholder(tf.float32, shape=[None, MAX_SFPD])
	...
	W4 = tf.Variable(tf.random_normal([1000, MAX_SFPD]))
	b4 = tf.Variable(tf.constant(0.0, shape=[MAX_SFPD]))
	...
	errors = [0]*(((train_len / batch_size + 10) * steps) / 10)
